...
FROM emscripten/emsdk:5.0.2 AS emsdk-base
ARG EXTRA_CFLAGS
ARG EXTRA_LDFLAGS
ARG FFMPEG_ST
ARG FFMPEG_MT
...
RUN apt-get update && \
      apt-get install -y pkg-config autoconf automake libtool ragel xxd

# Build x264
FROM emsdk-base AS x264-builder
ADD https://code.videolan.org/videolan/x264.git /src
...
ENV LIBVPX_BRANCH=v1.16.0
ADD https://chromium.googlesource.com/webm/libvpx.git#$LIBVPX_BRANCH /src
...
ADD https://gitlab.xiph.org/xiph/ogg.git#$OGG_BRANCH /src
...
#ENV THEORA_BRANCH=v1.2.0
#ADD https://gitlab.xiph.org/xiph/theora.git#$THEORA_BRANCH /src
...
#ADD https://gitlab.xiph.org/xiph/vorbis.git#$VORBIS_BRANCH /src
...
#ADD https://github.com/webmproject/libwebp#$LIBWEBP_BRANCH /src
